...
for (let i = 0; i < grid[0].length; i++) {
  startLocations.push({ x: i, y: 0, direction: "down" });
}
// start from left
for (let i = 0; i < grid.length; i++) {
  startLocations.push({ x: 0, y: i, direction: "right" });
}
// start from right
for (let i = 0; i < grid.length; i++) {
  startLocations.push({ x: grid[0].length - 1, y: i, direction: "left" });
}
// start from bottom
for (let i = 0; i < grid[0].length; i++) {
  startLocations.push({ x: i, y: grid.length - 1, direction: "up" });
}

const tileCounts = [];
for (let startLoc of startLocations) {
  let queue = [startLoc];
  ...
  tileCounts.push(seenTiles.size);
}

// print the maximum value of tileCounts
console.log(Math.max(...tileCounts));
